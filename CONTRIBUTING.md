...
We expect benchmarks that are realistic for embedded and FPGA systems, featuring multi-stage pipelines that can lead to multiple variants of the same benchmark for performance and energy consumption analysis.
...
At least a CPU and an HLS version must be ready to use. You could synthesize the HLS design using your target toolchain and ensure it works (e.g., verify via co-simulation).
...
Open a PR against the `main` branch of the upstream repository. Make sure to provide a clear summary of your benchmark, instructions on how to run your scripts, and a link back to your original Issue using GitHub's closing keywords (e.g., type `Resolves #12` or `Closes #12` in your PR description).
