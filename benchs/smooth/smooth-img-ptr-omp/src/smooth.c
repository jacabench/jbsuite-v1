 ...
 * 
 * 	Version using pointers and malloc and openmp for smooth and smooth_reuse1 versions
 * 
...
void smooth(uint8 *in, uint8 *out, int sX, int sY) {
	...
	#if NUM_THREADS == 0
	#pragma omp parallel for private(i, sum) collapse(2)
	#else
	#pragma omp parallel for private(i, sum) num_threads(NUM_THREADS) collapse(2)
	#endif
	for (j=0; j < sY-2; j++) {
		for (i=0; i < sX-2; i++) {
			sum = 0;
...
void smooth_reuse1(uint8 *in, uint8 *out, int sX, int sY) {
	...
	#if NUM_THREADS == 0
	#pragma omp parallel for private(i, sum, p11, p12, p13, p21, p22, p23, p31, p32, p33) 
	#else
	#pragma omp parallel for private(i, sum, p11, p12, p13, p21, p22, p23, p31, p32, p33)  num_threads(NUM_THREADS) 
	#endif
	for (j=0; j < sY-2; j++) {
		p11 = in[j*sX+0];
		p12 = in[j*sX+1];
