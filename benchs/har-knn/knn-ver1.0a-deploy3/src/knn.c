...
*/
CLASS_ID_TYPE normalize_classify(Point *new_point) {
	...
	// knowledge base of kNN statically initialized
	...
	// classify
   	...
   	instance_class = knn_classifyinstance_3(new_point, known_points);
	#else
    instance_class = knn_classifyinstance(new_point, known_points);
	#endif
...
void get_3_NN(Point *new_point, Point *known_points,
...
void select_k_nearest2(BestPoint *dist_points) {
	...
	for(int i = 0; i < K; i++) {
		int swapped = 0;
        for(int j = NUM_KNOWN_POINTS-i-1; j >=0; j--) {
