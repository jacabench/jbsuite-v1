...
void get_3_NN(Point *new_point, Point *known_points, int num_points,
    ...
    #if NUM_THREADS == 0
	#pragma omp parallel for private(j, i, distance) //shared(best_points)
	#else
	#pragma omp parallel for private(j, i, distance) num_threads(NUM_THREADS)
	#endif
...
void get_k_NN(Point *new_point, Point *known_points, int num_points,
    ...
    // training dataset (knowledge base)
    #if NUM_THREADS == 0
	#pragma omp parallel for private(i, j, distance) shared(best_points)
	#else
	#pragma omp parallel for private(i, j, distance) num_threads(NUM_THREADS) shared(best_points)
	#endif
    for (i = 0; i < num_points; i++) {
        distance = (DATA_TYPE) 0.0;
