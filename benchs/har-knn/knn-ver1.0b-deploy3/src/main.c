...
*	static				From file
*	----------------------------------------
*	train1_norm.dat 	static as READ = 2
*	----------------------------------------
*	test1_notnorm.dat	test2_notnorm.dat
*/

#ifndef READ
	#define READ 4 //2 or 4: read from .dat
#endif
...
#if READ == 2 // data embedded in program, arrays statically filled
...
int main() {
	...
	#if READ == 2 // data embedded in program
		printf("All statically initialized from data in files\n");
	#elif READ == 4  // data read from .dat files
		printf("Training vectors statically initialized from data in files\n");
    ...
    for (int i = 0; i < num_new_points; i++) {
		...
		// normalize the point and classify it!
		CLASS_ID_TYPE instance_class = normalize_classify(new_point);
