...
int main() {
    ...
    for (int i = 0; i < num_new_points; i++) {
		...
		CLASS_ID_TYPE instance_class = features_normalize_classify(&new_instances[index]);

		#if ACCURACY
			if(classid != instance_class) fail++;
		#endif
		
        // The following outputs the inferred class
        // In practice, and especially in streaming operation, 
		// the class is output to the subsequent stages of the application
		// for now: output the inferred class of the instance
		put(&instance_class, 0, "class id:");
    }
