...
int read_data_instances(char *file_name, int num_sensor_data, int num_samples, Instance *instances) 
{
	...
	if (num_samples <= 0 || num_sensor_data <= 0) {
		printf ("** Error: trying to read a negative or zero number of words.\n");
		exit (1);
	}

	for (int i = 0; i < num_samples; i++) {
		for (int j = 0; j < num_sensor_data; j++) {
			#if DT == 1
			success = fscanf (input_fp, "%lf", &(instances[i].data[j]));
			#elif DT == 2
			success = fscanf (input_fp, "%f", &(instances[i].data[j]));
			#endif
		}
		int value;
		success = fscanf (input_fp, "%d", &value);
		instances[i].classification_id = (CLASS_ID_TYPE) value;
		//printf("read %d\n", instances[i].classification_id);
	}  
