...
int main(void) {
 	...
 	type_t *W2 = (type_t *) malloc(M2 * C2 * R2 * sizeof(type_t));
 	type_t *B2 = (type_t *) malloc(M2 * sizeof(type_t));
 	type_t *W3 = (type_t *) malloc(M3 * C3 * R3 * sizeof(type_t));
 	type_t *B3 = (type_t *) malloc(M3 * sizeof(type_t));
 	type_t *W4 = (type_t *) malloc(M4 * C4 * R4 * sizeof(type_t));
 	type_t *B4 = (type_t *) malloc(M4 * sizeof(type_t));
 	type_t *W5 = (type_t *) malloc(M5 * C5 * R5 * sizeof(type_t));
 	type_t *B5 = (type_t *) malloc(M5 * sizeof(type_t));
 	type_t *W_fc1 = (type_t *) malloc(960 * sizeof(type_t));
 	type_t *B_fc1 = (type_t *) malloc(6 * sizeof(type_t));
 	...
 	all_weights_loaded &= load_and_convert_weights("W2.bin", W2, M2 * C2 * R2);
 	all_weights_loaded &= load_and_convert_weights("B2.bin", B2, M2);
 	all_weights_loaded &= load_and_convert_weights("W3.bin", W3, M3 * C3 * R3);
 	all_weights_loaded &= load_and_convert_weights("B3.bin", B3, M3);
 	all_weights_loaded &= load_and_convert_weights("W4.bin", W4, M4 * C4 * R4);
 	all_weights_loaded &= load_and_convert_weights("B4.bin", B4, M4);
 	all_weights_loaded &= load_and_convert_weights("W5.bin", W5, M5 * C5 * R5);
 	all_weights_loaded &= load_and_convert_weights("B5.bin", B5, M5);
 	all_weights_loaded &= load_and_convert_weights("W_fc1.bin", W_fc1, 960);
 	all_weights_loaded &= load_and_convert_weights("B_fc1.bin", B_fc1, 6);
 	...
 	for (int i = 0; i < num_samples; ++i) {
 	 	...
 	 	cnn(I1, W1, B1, W2, B2, W3, B3, W4, B4, W5, B5, W_fc1, B_fc1, O_final);
