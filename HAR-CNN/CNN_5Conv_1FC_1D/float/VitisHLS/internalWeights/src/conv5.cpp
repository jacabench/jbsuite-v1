...
void conv_1d_4(type_t I[C4 * H4], type_t const W[M4 * C4 * R4], type_t const B[M4], type_t O[M4 * E4]) {
 	for(int m = 0; m < M4; m++) {
 		for(int x = 0; x < E4; x++) {
 			O[x + m * E4] = B[m];
 			#pragma HLS PIPELINE II=1
...
void conv_1d_5(type_t I[C5 * H5], type_t const W[M5 * C5 * R5], type_t const B[M5], type_t O[M5 * E5]) {
 	for(int m = 0; m < M5; m++) {
 		for(int x = 0; x < E5; x++) {
 			O[x + m * E5] = B[m];
 			#pragma HLS PIPELINE II=1
...
void fc_layer_1(type_t input[FC1_IN_SIZE], type_t const W[FC1_IN_SIZE * FC1_OUT_SIZE], const type_t B[FC1_OUT_SIZE], type_t output[FC1_OUT_SIZE]) {
